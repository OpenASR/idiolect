...
In our experience as users and maintainers of the plugin, usability challenges typically arise in a few areas. We discuss various categories of errors and our efforts to address them.
...
We draw a distinction between capability discovery and intent recognition. Users previously familiar with the IDE capabilities may wish to invoke or bind a specific action directly, but may not know the command binding. The first and foremost way to improve discovery is through documentation. However keeping documentation in sync with capabilities can be a challenging task, and users are unlikely to read verbose documentation. To address this issue, we autogenerate documentation describing the action and their descriptions, by preprocessing the action list and assigning natural language descriptions from the IntelliJ Platform source code.
...
Idiolect can try to anticipate users actions and give users additional action bindings. However, there will always be some functionality that is best implemeneted by downstream developers. In addition to end-user configurability, Idiolect is designed to be extensible by external plugin developers and can be used by other IntelliJ Platform plugins to programmatically define their own commands and recognition handlers. We provide a simple message passing API for plugins to communicate with Idiolect, and a DSL for defining custom commands.

\subsection{Error Recovery}\label{sec:error}

Another common usability barrier is when the speech is transcribed accurately, but the utterance does not correspond to an actionable command, possibly due to stopwords or unexpected text. In short, if a given phrase, e.g., ``open uh foo java'' is received, we attempt to repair the utterance. At the user's discretion, or in the case of ambiguity when the phrase has multiple plausible alternatives, we can either visually or verbally prompt the user to select from a set of alternate phrases, e.g., ``Did you mean \textlangle a\textrangle open file foo.java, \textlangle b\textrangle open folder foo/java, or \textlangle c\textrangle something else?''

In keeping with the design principle of configurability, we allow users to define custom grammars and bind phrases to actions. In many cases however, there are verbal fillers and extraneous text which cannot be parsed directly. When the utterance is one or two tokens away from a command, we attempt to repair the utternace using error recovery.

We address the issue of recognition errors by incorporating Considine et al.'s (2022) work on Tidyparse, which supports supports defining and recognizing context-free and mildly context-sensitive grammars and computing language edit distance. Tidyparse implements a novel approach to error correction based on the theory of context-free language reachability, finite field arithmetic, conjunctive grammars and Levenshtein automata. We use a SAT solver to find the smallest edit transforming a string outside the language to a string inside the langauge.
...
We then performed an intervention, by switching to a new voice recognition engine, adding many usability improvements described in Sec.~\ref{sec:usability} and are collecting data on the number of downloads of the new version -- the results of this experiment are ongoing but promising.

\section{Results}

\includegraphics[width=0.45\textwidth]{downloads.png}

Downloads of the plugin have decreased over time since the plugin's initial release. We conjecture this relates to the fact that plugin was not updated for several years, and thus incompatible with the latest versions of IntelliJ Platform. After implementing the usability improvements described in Sec.~\ref{sec:usability}, we re-released the plugin in 2023.
